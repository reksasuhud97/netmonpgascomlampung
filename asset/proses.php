...
elseif (isset($_SESSION['admin'])) {
	// Tambahh kantor
	if (isset($_POST['add_kantor'])) {
		$kantor = mysqli_real_escape_string($conn, $_POST['name_st']);
		...
		$pusat = 0;
		$id = "NULL";
		if (!empty($kantor) || !empty($kd_telp) || !empty($no_telp) || !empty($add)) {
			$q_cek = $conn->query("SELECT*FROM blok WHERE name_blok='$kantor'");
			if ($q_cek->num_rows!=0) { // Cek ada nama kantor yg sama ga
				echo "<script>window.alert('Sudah ada kantor dengan nama \"$kantor\"');
						window.location=('../add_kantor');</script>"; // aksi ada
			} else {
				// aksi ga ada
				$sql = "INSERT INTO blok (id_blok, name_blok, telp_blok, add_blok, pusat_client) VALUES (?, ?, ?, ?, ?)";
				if ($statement = $conn->prepare($sql)) {
					$statement->bind_param(
						"isssi", $id, $kantor, $telp, $add, $pusat);
					if ($statement->execute()) {
						header("location:../add_kantor&st=1");
					} else {
						header("location:../add_kantor&st=2");
					}
				} else {
					header("location:../add_kantor&st=2");
				}
			...
			header("location:../add_kantor&st=3");
		}
	}
	// Tambah client
	elseif (isset($_POST['add_client'])) {
		...
		$blok = mysqli_real_escape_string($conn, $_POST['kantor']);
		...
		if (!empty($ip) || !empty($name) || !empty($blok)) {
			...
			if ($statement = $conn->prepare($sql)) {
				$statement->bind_param(
					"sss", $ip, $name, $blok);
				if ($statement->execute()) {
					header("location:../kantor&id=$blok&st=1");
				} else {
					header("location:../kantor&id=$blok&st=2");
				}
			} else {
				header("location:../kantor&st=2");
			}
		} else {
			header("location:../kantor&st=3");
		}
	}
	// Delete Client
	elseif (isset($_POST['del_cln'])) {
		...
		if ($delete->execute()) {
			header("location:../kantor&id=$id_blok&st=4");
		} else {
			header("location:../kantor&id=$id_blok&st=5");
		}
	}
	// Edit kantor
	elseif (isset($_POST['edit_kantor'])) {
		$id_st = mysqli_real_escape_string($conn, $_POST['id_st']);
        $kantor = mysqli_real_escape_string($conn, $_POST['name_st']);
        $kd_telp = mysqli_real_escape_string($conn, $_POST['kd_telp_st']);
        $telp = mysqli_real_escape_string($conn, $_POST['telp_st']);
        $add = mysqli_real_escape_string($conn, $_POST['add_st']);
        if (!empty($kantor) || !empty($kd_telp) || !empty($no_telp) || !empty($add) || !empty($id_st)) {
            $sql = "UPDATE blok SET name_blok=?,telp_blok=?, add_blok=? WHERE id_blok='$id_st'";
            if ($statement = $conn->prepare($sql)) {
                $statement->bind_param(
                    "sss", $kantor, $telp, $add);
                if ($statement->execute()) {
                    header("location:../kantor&id=$id_st&st=1");
                } else {
                    header("location:../kantor&id=$id_st&st=2");
                }
            } else {
                header("location:../kantor&st=2");
            }
    ...
    //Delete kantor dan semua isi client
    elseif (isset($_GET['del_st'])) {
		...
		if ($del_blok->execute() && $del_cln->execute()) {
			header("location:../kantor&st=4");
		} else {
			header("location:../kantor&st=5");
		}
	}
	// Del log
